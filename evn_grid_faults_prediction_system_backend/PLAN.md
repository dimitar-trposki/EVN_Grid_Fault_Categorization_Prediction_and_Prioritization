...
- [ ] **Repository custom methods** — `WeatherDataRepository`: `findLatestByLocationId`, `findByLocationIdAndRecordedAtBetween`
- [ ] **Request DTOs** — `WeatherFetchRequest` (locationId) (as `record`)
- [ ] **Response DTOs** — `WeatherDataResponse` (temperature, windSpeed, humidity, precipitation, condition, recordedAt) (as `record`)
- [ ] **Service interface** — `WeatherService`: `fetchAndStore(Long locationId)`, `getLatest(Long locationId)`, `getHistory(Long locationId, LocalDateTime from, LocalDateTime to)`
- [ ] **Service implementation** — `WeatherServiceImpl`: calls `WeatherClient` (WebClient, URL from `weather.api.base-url` + `weather.api.key`), maps response to `WeatherData`, saves
- [ ] **Controller + endpoints** — `WeatherController`: `POST /api/v1/weather/fetch` (ADMIN/OPERATOR), `GET /api/v1/locations/{id}/weather`, `GET /api/v1/locations/{id}/weather/history`
